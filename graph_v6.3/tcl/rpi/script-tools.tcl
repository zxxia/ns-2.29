...
#  create-mice-over-reno   creates a source that periodically sends a 
...
#                          file with exponential interrequest times
#                          and pareto (note: PARETO!!) file sizes, but uses 
#                          caller specified src and sink agent classes.
...
#         Yong Xia         added create-mice-over-sth  2/15/06
...
# and file size.)
proc create-mice-over-reno { n0 n1 { interrequest 5 } \
...
# d != days.   d means nothing.
# y != years.  y means nothing
#
...
# Opposite of t2sec.  This takes a time in seconds and 
# converts it into the most appropriate ns-2 time unit.
# For example:
#  arg     returned
#   5       5s
#   -5     -5s
...
proc sec2t { sec } {
  ...
  if { $s >= 1 } {
    return "[expr $sign * $s]s"
  }
  if { $s >= 0.001 } {
    return "[expr $sign * $s * 1000]ms"
  }
  if { $s >= 1.e-6 } {
    return "[expr $sign * $s * 1e6]us"
  }
  if { $s >= 1.e-9 } {
    return "[expr $sign * $s * 1.e9]ns"
  }
  else { 
    return "[expr $sign * $s * 1.e12]ps"
  }
}
